  ...
  const client = new Client({
    ...
    puppeteer: {
      ...
      args: [
        ...
        '--disable-gpu',
        '--disable-quic',
        // WhatsApp Web spawns a transient cross-origin iframe
        // (webtp.whatsapp.net/pdf-viewer) during load. With site isolation on,
        // that iframe is a SEPARATE CDP target that is destroyed within ~1s.
        // attachEventListeners()'s exposeFunction (addScriptToEvaluateOnNewDocument
        // / addBinding) then races that dying target and throws "Target closed",
        // so 'ready' never fires. Keeping cross-origin frames in-process removes
        // the separate target and the race.

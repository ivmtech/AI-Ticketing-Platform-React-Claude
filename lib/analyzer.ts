...
function buildPrompt(groupName: string, transcript: string): string {
  return [
    ...
    '5. confidence (0-1) - how sure are you of the resolved verdict? Use <0.7 if the conversation is ambiguous, transcript is incomplete, or you are guessing.',
    '',
...
function buildBatchPrompt(prepared: Array<{ groupName: string; transcript: string }>): string {
  ...
  return [
    ...
    '5. confidence (0-1) - use <0.7 if the conversation is ambiguous, incomplete, or you are guessing',
    '',
    sections,
    '',
